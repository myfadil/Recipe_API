const { getCommentById, postComment, putComment, deleteComment } = require("../model/CommentModel")
...
const CommentController = {
    getDataById: async (req, res, next) => {
        try {
            ...
            if (!dataCommentId.rows[0]) {
                return res.status(200).json({ "status": 200, "message": "comment not found", data: [] })
            }

            return res.status(200).json({ "status": 200, "message": "get comment success", data: dataCommentId.rows})
        } catch (err) {
    ...
    postData: async (req, res, next) => {
        ...
        try {
            ...
            return res.status(200).json({ "status": 200, "message": "Comment success", data })
        } catch (err) {
            return res.status(404).json({ "status": 404, "message": err.message })
        }
    },
    ...
    putData: async (req, res, next) => {
        try {
            ...
            return res.status(200).json({ "status": 200, "message": "update comment success", data })
        } catch (err) {
            return res.status(404).json({ "status": 404, "message": err.message })
